...
从 **Excel** 读取「产品编号」「客户型号」两列，支持手动纠错后，按产品编号创建文件夹及 YG 子目录，并从客户文件目录中按客户编号复制对应文件夹/PDF 到各 YG。
...
4. **选择客户文件所在目录**：该目录及子目录中名称包含「客户编号」（客户型号破折号前数字）的文件夹和 PDF 会被复制到对应 `产品编号/YG`。
5. 点击 **开始执行**，查看日志。
...
- 复制规则：在客户文件目录中，**名称包含该客户编号**的文件夹和 PDF 会被复制到对应产品编号下的 `YG` 目录。
